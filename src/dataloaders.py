...
class RandomCrop3D(object):
    ...
    def __init__(self, output_size, is_random=True, is_test=False, n_classes=9, is_normalize=False):
        ...
        # Check if instance of a tuple or int
    ...
    def __call__(self, sample):
        ...
        # Create new image
        image = image[patch_x: patch_x + new_h, patch_y: patch_y + new_w, patch_z: patch_z + new_d]
        # Create new lab
        if len(lab.shape) == 3:
            lab = lab[patch_x: patch_x + new_h, patch_y: patch_y + new_w, patch_z: patch_z + new_d]
        else:
            lab = lab[:, patch_x: patch_x + new_h, patch_y: patch_y + new_w, patch_z: patch_z + new_d]
...
class LocalisationDataLoaderRun(Dataset):
    """
    Localisation dataset for semantic segmentation
    """

    def __init__(self, run_input, root_dir, is_augment=False, transform=None):
        """
        Constructor
...
class RandomCrop3DRun(object):
    ...
    def __call__(self, sample):
        ...
        if self.is_test:
            new_h, new_w, new_d = h, w, d
            # print("original image dimensions ", h, w, d)
        else:
            new_h, new_w, new_d = self.output_size
            # print("target image dimensions ", new_h, new_w, new_d)
        ...
        if self.is_random:
            ...
            coords_x, coords_y, coords_z = np.meshgrid(np.arange(0, w) - (w - 1) / 2,
                                                       np.arange(0, h) - (h - 1) / 2,
                                                       np.arange(0, d) - (d - 1) / 2)
            coords_x = abs(np.round(np.mean(coords_x * np.sum(lab, axis=0)) + (h - 1) / 2))
            coords_y = abs(np.round(np.mean(coords_y * np.sum(lab, axis=0)) + (w - 1) / 2))
            coords_z = abs(np.round(np.mean(coords_z * np.sum(lab, axis=0)) + (d - 1) / 2))
