...
def get_cropped_brain_body(input_size_, output_size_, img_input_, seg_output_):
    ...
    for i in np.arange(0, b):
        for j in [1, 2]:  # only interested in body and brain
            ...
            # Add to variable
            img_cropped_input_[i, j - 1] = img_input_[i, 0, patch_x:patch_x + new_h, patch_y:patch_y + new_w, patch_z:patch_z + new_d]
            seg_cropped_output_[i, j - 1] = seg_output_[i, j + 1, patch_x:patch_x + new_h, patch_y:patch_y + new_w, patch_z:patch_z + new_d]

    return torch.from_numpy(img_cropped_input_), torch.from_numpy(seg_cropped_output_), patch_coords_, mask_exists_
...
def create_grid(vol_size, batch_size, patch_size, patch_coords, id_c):
    ...
    for i_b in range(batch_size):
        if i_b == 0:
            new_grid = grid[i_b:i_b + 1, :, patch_coords[i_b, id_c, 0]:patch_coords[i_b, id_c, 0] + patch_size[0], patch_coords[i_b, id_c, 1]:patch_coords[i_b, id_c, 1] + patch_size[1], patch_coords[i_b, id_c, 2]:patch_coords[i_b, id_c, 2] + patch_size[2]]
        else:
            temp_ = grid[i_b:i_b + 1, :, patch_coords[i_b, id_c, 0]:patch_coords[i_b, id_c, 0] + patch_size[0], patch_coords[i_b, id_c, 1]:patch_coords[i_b, id_c, 1] + patch_size[1], patch_coords[i_b, id_c, 2]:patch_coords[i_b, id_c, 2] + patch_size[2]]
...
class ArgumentsTrainTestLocalisation():
    def __init__(self,
                 ...
                 run_input='/data/projects/localisation/data/run/rec2loc.nii.gz',
                 exp_name='test',
                 task_net='unet_3D',
                 cls_net='cls_3D',
                 n_classes=2):
