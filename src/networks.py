...
from src.dataloaders import LocalisationDataLoaderRun, ToTensorRun, RandomCrop3DRun
...
class LocalisationNetwork3DMultipleLabels(object):
    def __init__(self, args):
        ...
        transformed_dataset_run = LocalisationDataLoaderRun(
            run_input=args.run_input,
            root_dir=args.root_dir,
            is_augment=False,
            transform=transforms.Compose([RandomCrop3DRun(output_size=(args.crop_width,
                                                                       args.crop_height,
                                                                       args.crop_depth),
                                                          is_random=False),
                                          ToTensorRun()]))
    ...
    def test(self, args, mode):
        ...
        for i, data_point in enumerate(self.dataloaders['test']):
            ...
            def save_nii_img_seg(args_, name_, img_gt_, seg_gt_, seg_pr_, img_aff_, seg_aff_, ind, case_id_):
                ...
                img_tmp_info = nib.load(
                    args_.results_dir + name_ + '_img-' + str(ind) + '-' + str(case_id_) + '.nii.gz')
            ...
            def save_nii_img_seg_prob(args_, name_, img_gt_, seg_gt_, seg_pr_, img_aff_, seg_aff_, prob_out_, n_labels_,
                ...
                for l in range(n_labels_):
                    prob_out = nib.Nifti1Image(prob_out_.cpu().data.numpy()[0, l + 1, :, :, :], img_tmp_info.affine,
                                               img_tmp_info.header)
                    nib.save(prob_out, args_.results_dir + name_ + '_pr-' + str(l + 1) + ' _ ' + str(ind) + '-' + str(
                        case_id_) + '.nii.gz')
    ...
    def run(self, args, mode):
        ...
        for i, data_point in enumerate(self.dataloaders['run']):
            ...
            time_elapsed = time.time() - start_time
            print("Brain localization predicted in %.0fm %.2fs" %
                  (time_elapsed // 60, time_elapsed % 60))
            print("Brain localization predicted in ", time_elapsed)
            ...
            x_cm, y_cm, z_cm = ndimage.measurements.center_of_mass(seg_pr)

            self.x_cm = np.round(x_cm, 4)
            self.y_cm = np.round(y_cm, 4)
            self.z_cm = np.round(z_cm, 4)

            # # # # # # # # # # # # # # # # # # # # # # # #
            def save_nii_img_seg(args_, _name, img_gt_, seg_pr_, img_aff_, ind, case_id_):

                gt_img = np.array(img_gt_, np.int64)
                pr_lab = np.array(seg_pr_, np.int64)

                # Save as nib file - IMG GT
                gt_img = nib.Nifti1Image(img_gt_, img_aff_)
                nib.save(gt_img, args_.results_dir + _name + '_img-' + str(ind) + '-' + str(case_id_) + '.nii.gz')
                img_tmp_info = nib.load(args_.results_dir + _name + '_img-' + str(ind) + '-' + str(case_id_) + '.nii.gz')
            ...
            # # # # # # # # # # # # # # # # # # # # # # # #
            def save_nii_img_seg_no(args_, _name, img_gt_, seg_pr_, ind):

                gt_img = np.array(img_gt_, np.int64)
                pr_lab = np.array(seg_pr_, np.int64)

                # Save as nib file - IMG GT
                gt_img = nib.Nifti1Image(img_gt_, np.eye(4))
                nib.save(gt_img, args_.results_dir + _name + '_img-' + str(ind) + '.nii.gz')
                img_tmp_info = nib.load(args_.results_dir + _name + '_img-' + str(ind) + '.nii.gz')

                # Save as nib file - SEG PR
                pr_lab = nib.Nifti1Image(seg_pr_, np.eye(4))
                nib.save(pr_lab, args_.results_dir + name_ + '_seg_pr-' + str(ind) + '.nii.gz')

            # # # # # # # # # # # # # # # # # # # # # # # #
            def save_nii_img_seg_prob(args_, _name, img_gt_, seg_pr_, img_aff_, prob_out_, n_labels_, ind, case_id_):
                ...
                # Save as nib file - SEG PR
                pr_lab = nib.Nifti1Image(seg_pr_, img_tmp_info.affine, img_tmp_info.header)
                nib.save(pr_lab, args_.results_dir + _name + '_seg_pr-' + str(ind) + '.nii.gz')
            ...
            def figstring(args_, name):
                date_path = datetime.today().strftime("%Y-%m-%d")
                timestamp = f"{datetime.today().strftime('%H-%M-%S')}"

                if not os.path.isdir(args_.results_dir + date_path):
                    os.mkdir(args_.results_dir + date_path)

                final_str_nii = f"{date_path}{os.sep}{timestamp}-{name}"
                final_str_no_ext = f"{date_path}{os.sep}{timestamp}-{name}"
                return final_str_nii

            name_ = figstring(args, "gadgetron-fetal-brain-localisation-mask")

            save_nii_img_seg_no(args, name_, img_gt, seg_pr, i)

            # # # # # # # # # # # # # # # # # # # # # # # #
            def displ_res_all(args_, img_gt_, seg_pr_, prob_out_, pos_, n_labels_):

                plot_range = n_labels_
                l_num = n_labels_

                cm_crd = ndimage.measurements.center_of_mass(seg_pr_)

                pos_x = int(np.round(cm_crd[0]))
                pos_y = int(np.round(cm_crd[1]))
                pos_z = int(np.round(cm_crd[2]))
                ...
                z = 1
                plt.subplot(M, N, z)
                plt.imshow(img_gt_[:, :, pos_z], cmap='gray')
                plt.title('XY: ORG')
                plt.colorbar()
                ...
                plt.subplot(M, N, z)
                plt.imshow(img_gt_[:, :, pos_z], cmap='gray')
                plt.imshow(seg_pr_[:, :, pos_z], alpha=0.5, vmin=0, vmax=l_num)
                plt.title('XY: PRED')
                plt.colorbar()
                ...
                #     z = z + 1
                #     plt.subplot(M, N, z)
                #     # plt.imshow(prob_out_.cpu().data.numpy()[0, l + 1, :, :, pos_z], vmin=0, vmax=100)
                ...
                z = z + 1
                plt.subplot(M, N, z)
                plt.imshow(img_gt_[:, pos_y, :], cmap='gray')
                plt.title('XZ: ORG')
                plt.colorbar()
                ...
                plt.subplot(M, N, z)
                plt.imshow(img_gt_[:, pos_y, :], cmap='gray')
                plt.imshow(seg_pr_[:, pos_y, :], alpha=0.5, vmin=0, vmax=l_num)
                plt.title('XZ: PRED')
                plt.colorbar()
                ...
                #     z = z + 1
                #     plt.subplot(M, N, z)
                #     # plt.imshow(prob_out_.cpu().data.numpy()[0, l + 1, :, pos_y, :], vmin=0, vmax=100)
                #     plt.imshow(prob_out_.cpu().data.numpy()[0, l + 1, :, pos_, :], vmin=0, vmax=100)
                #     plt.imshow('XZ: PROB')
                #     plt.colorbar()
                ...
                z = z + 1
                plt.subplot(M, N, z)
                plt.imshow(img_gt_[pos_x, :, :], cmap='gray')
                plt.title('YZ: ORG')
                plt.colorbar()
                ...
                plt.subplot(M, N, z)
                plt.imshow(img_gt_[pos_x, :, :], cmap='gray')
                plt.imshow(seg_pr_[pos_x, :, :], alpha=0.5, vmin=0, vmax=l_num)
                plt.title('YZ: PRED')
                plt.colorbar()

                # timestamp = f"{datetime.today().strftime('%H-%M-%S')}"
                # plt.savefig(args_.results_dir + "/" + timestamp + "-gadgetron-localiser-result.png")
                date_path = datetime.today().strftime("%Y-%m-%d")
                timestamp = f"{datetime.today().strftime('%H-%M-%S')}"
                plt.savefig(args_.results_dir + "/" + date_path + "/" + timestamp + "-gadgetron-localiser-result.png")

                # for l in range(plot_range):
                #     z = z + 1
                #     plt.subplot(M, N, z)
                #     # plt.imshow(prob_out_.cpu().data.numpy()[0, l + 1, pos_x, :, :], vmin=0, vmax=100)
                #     plt.imshow(prob_out_.cpu().data.numpy()[0, l + 1, pos_, :, :], vmin=0, vmax=100)
                #     plt.title('YZ: PROB')
                #     plt.colorbar()
            ...
            if mode > 0:
                print(name_)
                displ_res_all(args, img_gt, seg_pr, out_prob, args.crop_depth // 2, self.n_labels)
