...
=======
>>>>>>> 1e7f6dcf9f80caa4f813333efa0b8af15d01733d
dtype_mapping = {
...
class ImageHeader(FlagsMixin, EqualityMixin, ctypes.Structure):
    ...
    def from_acquisition(acquisition, **kwargs):
        ...
        for field in kwargs:
            # field = ctypes.c_float * len(field)
...
class Image(FlagsMixin):
    __readonly = ('data_type', 'matrix_size', 'channels', 'user_float')
    ...
    def deserialize_from(read_exactly):
        ...
        # def calculate_number_of_entries(nchannels, xs, ys, zs, usrflt):
        def calculate_number_of_entries(nchannels, xs, ys, zs):
            return nchannels * xs * ys * zs
            # return nchannels * xs * ys * zs + usrflt

        # nentries = calculate_number_of_entries(image.channels, *image.matrix_size, image.user_float)
    ...
    @staticmethod
    def from_array_wcm(rx, ry, rz, tx, ty, tz, array, acquisition=Acquisition(), transpose=True, **kwargs):

        def input_shape_to_header_format_wcm(array):
            def with_defaults_wcm(first=1, second=1, third=1, nchannels=1):
                return nchannels, (first, second, third)

            return with_defaults_wcm(*reversed(array.shape))

        def header_format_to_resize_shape_wcm(nchannels, first, second, third):
        ...
        nchannels, matrix_size = input_shape_to_header_format_wcm(array)

        arr = (ctypes.c_float * 8)(*range(8))
        ...
        image.data[:] = np.resize(array, header_format_to_resize_shape_wcm(nchannels, *matrix_size))
